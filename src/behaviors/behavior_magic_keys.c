...
void record_key(uint32_t keycode) {
    ...
    LOG_DBG("recorded key 0x%02x 0x%02x", ZMK_HID_USAGE_PAGE(keycode), ZMK_HID_USAGE_ID(keycode));
...
static uint32_t apply_magic_rules(const struct behavior_magic_keys_config *cfg, uint32_t keycode) {
    ...
    LOG_DBG("looking up key in magic rules: usage page 0x%02x keycode 0x%02x",
            ZMK_HID_USAGE_PAGE(keycode), ZMK_HID_USAGE_ID(keycode));
    ...
    for (int i = 0; i + 1 < cfg->magic_rules_len; i += 2) {
        if (cfg->magic_rules[i] == keycode) {
            ...
            LOG_DBG("found rule match: usage page 0x%02x keycode 0x%02x",
                    ZMK_HID_USAGE_PAGE(matched), ZMK_HID_USAGE_ID(matched));
            return matched;
        }
    }
...
static uint32_t apply_skip_magic_rules(const struct behavior_magic_keys_config *cfg,
    ...
    // where an even index is the input key and the odd index is the output key after it,
    // the result is at i + 1.
    LOG_DBG("looking up key in skip magic rules: usage page 0x%02x keycode 0x%02x",
            ZMK_HID_USAGE_PAGE(keycode), ZMK_HID_USAGE_ID(keycode));
